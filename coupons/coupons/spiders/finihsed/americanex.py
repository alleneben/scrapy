...
def categoryHandler(value):
    ...
    if m:
        return urllib.parse.urljoin('https://rewards.americanexpress.co.il/', re.search(r"\/link\/(.+?)\.aspx", value).group(0))
...
def mainHandler(value):
    ...
    if m:
        ...
        print(path)
        if re.search(r"^\/",path):
            uri =  urllib.parse.urljoin('https://rewards.americanexpress.co.il/', re.search(r"\/link\/(.+?)\.aspx", value).group(0))
        else:
            uri = path
        print(uri)
        return uri
...
class AmericanexSpider(CrawlSpider):
    name = 'americanexpress'
    undetectable = False
    wait = False
    allowed_domains = ['rewards.americanexpress.co.il']
    start_urls = ['https://rewards.americanexpress.co.il']
    brands = getBrands()
    rules = [
    ...
    def __init__(self, *args, **kwargs):
        super(AmericanexSpider, self).__init__(*args, **kwargs)
