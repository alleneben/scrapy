...
def itemHandler(value):
    ...
    if m:
        result = 'https://www.cal-store.co.il/'+ m.group(0)
        return result
...
class CalSpider(CrawlSpider):
    name = 'cal'
    undetectable = True
    wait = True
    elementId = 'cal-shop-brand'
    allowed_domains = ['cal-store.co.il']
    start_urls = ['https://www.cal-store.co.il']
    brands = getBrands()
    ...
    def __init__(self, *args, **kwargs):
        super(CalSpider, self).__init__(*args, **kwargs)
